...
EOnlineEnvironment::Type FOnlineSubsystemEOS::GetOnlineEnvironment() const
{
	FString SandboxId;
	EOnlineEnvironment::Type OnlineEnvironment = EOnlineEnvironment::Development;

	FParse::Value(FCommandLine::Get(), TEXT("EpicSandboxId="), SandboxId);

	if (SandboxId == TEXT("StageSandboxId")) // Epic's Stage sandbox
	{
		OnlineEnvironment = EOnlineEnvironment::Certification;
	}
	else if (SandboxId == TEXT("LiveSandboxId")) // Epic's Live sandbox
	{
